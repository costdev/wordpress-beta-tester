...
class WP_Beta_Tester {
	...
	public function beta_tester_dashboard() {
		...
		printf( wp_kses_post( '<p>' . __( 'Here are the <a href="%s" target="_blank">commits for the milestone</a>.', 'wordpress-beta-tester' ) . '</p>' ), sanitize_url( "https://core.trac.wordpress.org/query?status=closed&status=reopened&milestone=$milestone" ) );
		...
		if ( current_user_can( $capability ) ) {
			...
			printf( wp_kses_post( '<p>' . __( 'Head over to your <a href="%s">WordPress Beta Tester Settings</a> and make sure the <strong>beta/RC</strong> stream is selected.', 'wordpress-beta-tester' ) . '</p>' ), sanitize_url( $wpbt_settings_page ) );
		}
	}
